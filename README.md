<div align="center">

<!-- Dynamic Typing Header -->
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=32&duration=3000&pause=1000&color=00D9FF&center=true&vCenter=true&width=600&lines=Hey+there%2C+I'm+Shubham+Raj+%F0%9F%91%8B;Full+Stack+Developer;Python+%7C+Django+%7C+React;Building+real-world+solutions+%F0%9F%9A%80" alt="Typing SVG" />

<br/>

<img src="https://komarev.com/ghpvc/?username=shubbhamsingh&label=Profile+Views&color=00d9ff&style=flat-square" alt="Profile Views" />
&nbsp;
<a href="https://www.linkedin.com/in/shubham-raj840"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white" /></a>
&nbsp;
<img src="https://img.shields.io/badge/Open%20to%20Work-00C853?style=flat-square&logo=checkmarx&logoColor=white" />

</div>

---

## 👨‍💻 About Me

```python
class ShubhamRaj:
    def __init__(self):
        self.name        = "Shubham Raj"
        self.role        = "Full Stack Developer"
        self.university  = "Jagannath University, Jaipur (B.Tech CSE '27)"
        self.location    = "Jaipur, Rajasthan, India 🇮🇳"
        self.focus       = ["Python", "Django", "Full Stack Web Development"]
        self.building    = "UrbanShift — Real Estate & Relocation Platform"
        self.goal        = "Solving real-world problems through clean code"

    def say_hi(self):
        print("Thanks for dropping by! Let's build something great together.")

me = ShubhamRaj()
me.say_hi()
```

---

## 🛠️ Tech Stack

**Frontend**

<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45" height="45" alt="HTML5" title="HTML5"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45" height="45" alt="CSS3" title="CSS3"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="45" height="45" alt="JavaScript" title="JavaScript"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="45" height="45" alt="React" title="React"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg" width="45" height="45" alt="Bootstrap" title="Bootstrap"/>
</p>
<p>
  <sub>HTML5 &nbsp;&nbsp;&nbsp;&nbsp; CSS3 &nbsp;&nbsp;&nbsp;&nbsp; JavaScript &nbsp;&nbsp;&nbsp;&nbsp; React &nbsp;&nbsp;&nbsp;&nbsp; Bootstrap</sub>
</p>

**Backend**

<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45" height="45" alt="Python" title="Python"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/django/django-plain.svg" width="45" height="45" alt="Django" title="Django"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" width="45" height="45" alt="Node.js" title="Node.js"/>
</p>
<p>
  <sub>Python &nbsp;&nbsp;&nbsp;&nbsp; Django &nbsp;&nbsp;&nbsp;&nbsp; Node.js</sub>
</p>

**Database & Tools**

<p>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="45" height="45" alt="MongoDB" title="MongoDB"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45" height="45" alt="Git" title="Git"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45" height="45" alt="GitHub" title="GitHub"/>
  &nbsp;&nbsp;
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45" height="45" alt="VS Code" title="VS Code"/>
</p>
<p>
  <sub>MongoDB &nbsp;&nbsp;&nbsp;&nbsp; Git &nbsp;&nbsp;&nbsp;&nbsp; GitHub &nbsp;&nbsp;&nbsp;&nbsp; VS Code</sub>
</p>

---

## 🚀 Featured Projects

### 🏙️ UrbanShift — Real Estate & Relocation Platform
> A modern web platform to **Buy, Sell, Rent** properties and book **Packers & Movers** seamlessly.

- 🔧 **Tech:** JavaScript, HTML, CSS
- 💡 Built to simplify property search and relocation in Indian cities
- 📌 [View Repository →](https://github.com/shubbhamsingh/UrbanShift-Project)

---

### 🍕 Pizza Store — Web App
> A fully functional pizza ordering web application with dynamic cart management.

- 🔧 **Tech:** JavaScript, HTML, CSS
- 📌 [View Repository →](https://github.com/shubbhamsingh/Pizza_store_project)

---

### 🏆 Techvolution 2024 — Major Project
> Hackathon project built under Techvolution 2024 competition.

- 🔧 **Tech:** CSS, HTML
- 📌 [View Repository →](https://github.com/shubbhamsingh/Techvolution_2024_Major_Project)

---

### 📋 Techvolution 2024 — Minor Project
> A clean and responsive **Event Registration Form** with real-time client-side validation built for Techvolution 2024.

- 🔧 **Tech:** HTML5, CSS3, JavaScript, Bootstrap 5
- ✨ Features: Responsive design, real-time email/phone validation, interactive UI
- 🌐 [Live Demo →](https://shubbhamsingh.github.io/Techvolution_Minor_Project/)
- 📌 [View Repository →](https://github.com/shubbhamsingh/Techvolution_Minor_Project)

---

## 💼 Experience

**🖥️ Web Development Intern — YHills**
`Mar 2024 – Present`
- Completed intensive Full Stack Web Development training
- Built and deployed real-world minor and major projects
- Gained hands-on experience in React.js, Node.js, and MongoDB

---

## 📜 Certifications

| Certificate | Issuer | Year |
|---|---|---|
| 🏅 Full Stack Web Development | YHills | 2024 |
| 🛡️ Cyber Security | Cisco | 2024 |
| 🤖 Generative AI Literacy | LinkedIn Learning | 2024 |
| 📊 Data Analytics Workshop | GRAS | 2024 |

---

## 📊 GitHub Stats

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=shubbhamsingh&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00D9FF&icon_color=00D9FF&text_color=FFFFFF" height="160" />
&nbsp;
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=shubbhamsingh&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=00D9FF&text_color=FFFFFF" height="160" />

<br/><br/>

<img src="https://github-readme-streak-stats.herokuapp.com/?user=shubbhamsingh&theme=tokyonight&hide_border=true&background=0D1117&stroke=00D9FF&ring=00D9FF&fire=FF6B35&currStreakLabel=00D9FF" />

</div>

---

## 🤝 Let's Connect

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Shubham%20Raj-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/shubham-raj840)
[![GitHub](https://img.shields.io/badge/GitHub-shubbhamsingh-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/shubbhamsingh)

</div>

<div align="center">

---

*"Code is not just syntax — it's the solution to someone's real problem."*

⭐ **If you like my work, give a star to my repos!** ⭐

</div>
